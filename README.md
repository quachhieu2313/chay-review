...
4. **Hosting**: vào [render.com](https://render.com) → New → **Blueprint**, trỏ vào repo GitHub — Render sẽ tự đọc file `render.yaml` có sẵn trong repo để tạo Web Service (không cần tự nhập build/start command). Điền các biến môi trường Render yêu cầu: `DATABASE_URL`, `CLOUDINARY_URL`, và `DJANGO_SUPERUSER_USERNAME` / `DJANGO_SUPERUSER_EMAIL` / `DJANGO_SUPERUSER_PASSWORD` (tài khoản admin sẽ tự được tạo trong lúc build, không cần Shell hay kết nối DB thủ công).
